...
| 14 | Publication | A finished course exports as one standalone HTML reader containing course material only—never chat transcripts or Q&A. Before bundling, the learner may give a one-off prompt (translation, executive summary, restructuring, or anything else); a separate coding-agent session applies it to a temporary course copy, leaving the source course and its history unchanged. The derived artifact carries TIL-compatible metadata and body markers | Publication often needs a different language, framing, or level of compression than the learning workspace. A disposable transformation stage keeps that freedom without contaminating the canonical course or forcing unanchored conversation into the reading experience |
