...
| 16 | Course localization & release | Monolingual co-creation loop (zero token overhead during authoring); on-demand session or course translation generates localized sibling files (e.g. `session1.zh-CN.html`). Translated pages are independently refined via the same select-and-edit studio loop in that language. Terminology follows a 3-tier strategy (universal standards translated, emerging concepts annotated bilingual on first mention, unstable jargon/tooling retained in English); widgets preserve DOM IDs, classes, and script logic | Prevents 2x-3x token waste and brittle multi-language DOM synchronization during rapid authoring while preserving full interactive co-design and refinement capability for localized editions |
